...
const SearchUser = () => {
    ...
    const [phoneNumberSubmit, setPhoneNumberSubmit] = useState("");
    const [userDiv, setUserDiv] = useState(<>Enter User Phone Number To Locate User</>);
    ...
    return(
        ...
        <form id="search" onSubmit={(event) => {
            // write code here
            event.preventDefault();
            setPhoneNumberSubmit(phoneNumber);
                } 
        }>
            <fieldset>
              <label htmlFor="keywords" id="searchTitle">Search:</label>
                <input type="text" id="searchInput" placeholder="Enter User Phone Number" onChange={(event) => setPhoneNumber(event.target.value)}/>
            </fieldset>
            <button id="searchButton">SEARCH</button>
          </form>
          <>
          {users.map((user, index) => {
            if (user.phone_number === phoneNumberSubmit){
            console.log({user});
            return(
                <>  
                <h1 id="title">User</h1>
